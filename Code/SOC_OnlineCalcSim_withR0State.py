...
def kalman_fitler(time,current,voltage,OCV_array,dOCV_dSOC_array,SOC_array,Qnom,SOC_o,P1,sigma_bar,params,M,method_flag):
    R0 = params[0]
    R1 = params[1]
    C1 = params[2]
    R2 = params[3]
    ...
    y2[0] = 0
    x = np.array([[SOC[0]],[0],[0],[0]])
    K_track = []
    K_initial = np.array([[0],[0],[0],[0]])
    ...
    for k in range(1,len(time)):
        ...
        if method_flag == 0:
            F = np.array([[1,0,0,0],[0,1-dt/R1/C1,0,0],[0,0,1-dt/R2/C2,0],[0,0,0,0]])
            B = np.array([[-dt/Qnom/3600], [dt/C1], [dt/C2], [R0]])
            ...
            Q4 = np.power(R0*sigma_i,2) + np.power(current[k-1]*sigma_R0,2)
            ...
            F = np.array([[1,0,0,0],[0,np.exp(-dt/R1/C1),0,0],[0,0,np.exp(-dt/R2/C2),0],[0,0,0,0]])
            B = np.array([[-dt/Qnom/3600], [R1*(1-np.exp(-dt/R1/C1))], [R2*(1-np.exp(-dt/R2/C2))], [R0]])
            ...
            # not including sigma_V
            Q2 = np.power(dt/R1/R1/C1*np.exp(-dt/R1/C1)*x[1][0]*sigma_R1+1*(1-np.exp(-dt/R1/C1))*current[k-1]*sigma_R1-R1*dt/R1/R1/C1*np.exp(-dt/R1/C1)*current[k-1]*sigma_R1,2)+np.power(dt/R1/C1/C1*np.exp(-dt/R1/C1)*x[1][0]*sigma_C1-R1*dt/R1/C1/C1*np.exp(-dt/R1/C1)*current[k-1]*sigma_C1,2)+np.power(R1*(1-np.exp(-dt/R1/C1))*sigma_i,2)
            Q3 = np.power(dt/R2/R2/C2*np.exp(-dt/R2/C2)*x[2][0]*sigma_R2+1*(1-np.exp(-dt/R2/C2))*current[k-1]*sigma_R2-R2*dt/R2/R2/C2*np.exp(-dt/R2/C2)*current[k-1]*sigma_R2,2)+np.power(dt/R2/C2/C2*np.exp(-dt/R2/C2)*x[2][0]*sigma_C2-R2*dt/R2/C2/C2*np.exp(-dt/R2/C2)*current[k-1]*sigma_C2,2)+np.power(R2*(1-np.exp(-dt/R2/C2))*sigma_i,2)
            Q4 = np.power(R0*sigma_i,2) + np.power(current[k-1]*sigma_R0,2)
        ...
        z_k = voltage[k]
        y_k = OCV_fn_here(x_f[0][0]) - x_f[1][0] - x_f[2][0] - x_f[3][0]
...
sigma_SOC = 0.02 # 2 %
sigma_V = 0.05 # 50 mV
P = np.array([[np.power(sigma_SOC,2),0,0,0],[0,np.power(sigma_V,2),0,0],[0,0,np.power(sigma_V,2),0],[0,0,0,np.power(sigma_V,2)]])
sigma_i = 0.01 # 0.01 A
sigma_Q = 0.02 # 0.2 Ah

sigma_R0_bar = np.array([0.11,0.01,0.14]) * R0_bar
sigma_R1_bar = np.array([0.02,0.31,0.26]) * R1_bar
sigma_C1_bar = np.array([0.04,0.002,0.03]) * C1_bar
sigma_R2_bar = np.array([0.42,0.67,1.26]) * R2_bar
sigma_C2_bar = np.array([0.05,0.05,0.006]) * C2_bar
...
M = 10000000000
M = 1
result_KalmanFilter = kalman_fitler(t_data,i_data,V_data,OCV,dOCV_dSOC,SOC,Qnom,SOC_o,P,sigma_bar,params,M,method_flag)
...
K_KalmanFilter_4 = K_KalmanFilter[:,3,0]
...
axs[1].plot(t_data, OCV_data, marker='o', markersize=size_for_markers_plot,label='Actual OCV')
...
axs[3].plot(t_data, K_KalmanFilter_4, marker='o', markersize=size_for_markers_plot,label='Actual SOC')
axs[3].set_xlabel('Time (s)')
axs[3].set_ylabel('K4')
# axs[2].set_title("Kalman Gain vs Time")
axs[3].legend()
...
axs[1].set_xlabel('Time (s)')
axs[1].set_ylabel('K1 * (z_k - y_k)')
axs[1].set_title("K1 * (z_k - y_k)")
axs[1].legend()
axs[1].grid(True)

axs[2].plot(t_data,K_KalmanFilter_2*(V_data-V_KalmanFilter),marker='o', markersize=size_for_markers_plot,label='K*error')
axs[2].set_xlabel('Time (s)')
axs[2].set_ylabel('K2 * (z_k - y_k)')
axs[2].set_title("K2 * (z_k - y_k)")
axs[2].legend()
axs[2].grid(True)

axs[3].plot(t_data,K_KalmanFilter_3*(V_data-V_KalmanFilter),marker='o', markersize=size_for_markers_plot,label='K*error')
axs[3].set_xlabel('Time (s)')
axs[3].set_ylabel('K3 * (z_k - y_k)')
axs[3].set_title("K3 * (z_k - y_k)")
axs[3].legend()
axs[3].grid(True)

axs[4].plot(t_data,K_KalmanFilter_4*(V_data-V_KalmanFilter),marker='o', markersize=size_for_markers_plot,label='K*error')
axs[4].set_xlabel('Time (s)')
axs[4].set_ylabel('K4 * (z_k - y_k)')
axs[4].set_title("K4 * (z_k - y_k)")
axs[4].legend()
axs[4].grid(True)
