...
Io = df[df['Step_Index'] == 6]['Current(A)'].iloc[-1]
to = df[df['Step_Index'] == 6]['Step_Time(s)'].iloc[-1]
print('Voltage at end of previous step index: ',Vo)
print('Voltage at beginning of first step index: ', y_data_to_fit.iloc[0])
# Ro = (Vo-y_data_to_fit.iloc[0])/Io # use the below
Ro = (y_data_to_fit.iloc[0]-Vo)/(-Io)
delta_t = (to-x_data_to_fit.iloc[0])
...
r_squared = 1 - (ss_res / ss_tot)
print('R^2: ', r_squared)
param_track = [OCV, Ro, delta_t, a1, a2, tau1, tau2, r_squared]
df_export = pd.DataFrame([param_track],columns=["OCV","Ro", "delta_t", "a1","a2","tau1","tau2","r2"])
filename = f"LowCurrent_Sample{samNum}_{T}degC_params.csv"
...
# original data
fig, axs = plt.subplots(figsize=(8, 5), sharex=True)
axs.plot(x_data_to_fit,(y_data_to_fit-y_data_to_fit.iloc[size-1]),label='Data')
