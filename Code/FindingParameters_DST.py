...
def simulate_RC_voltage_online(t,i_data,R1,C1,R2,C2):
    ...
    for k in range(1,len(t)):
        dt = t.iloc[k] - t.iloc[k-1]
        V1[k] = V1[k-1]*np.exp(-dt/R1/C1) + i_data.iloc[k-1]*R1*(1-np.exp(-dt/R1/C1))
        V2[k] = V2[k-1]*np.exp(-dt/R2/C2) + i_data.iloc[k-1]*R2*(1-np.exp(-dt/R2/C2))
...
def online_voltage_fit(t, i_data, V_data, SOC_data, OCV_fn, Q, params):
    R0 = params[0]
    R1 = params[1]
    C1 = params[2]
    R2 = params[3]
    ...
    V = np.zeros_like(V_data, dtype=float)

    V_OCV = np.zeros_like(V_data, dtype=float)
    V_OCV[0] = OCV_fn(SOC_data.iloc[0])

    V0 = np.zeros_like(V_data, dtype=float)
    V0[0] = i_data.iloc[0] * R0
    ...
    for k in range(1,len(t)):
        dt = t.iloc[k] - t.iloc[k-1]
        V_OCV[k] = OCV_fn(SOC_data.iloc[k])
        V0[k] = i_data.iloc[k] * R0
        V1[k] = V1[k-1]*np.exp(-dt/R1/C1) + i_data.iloc[k-1]*R1*(1-np.exp(-dt/R1/C1))
        V2[k] = V2[k-1]*np.exp(-dt/R2/C2) + i_data.iloc[k-1]*R2*(1-np.exp(-dt/R2/C2))
...
V_fit_online, V_ocv_fit_online, V0_fit_online, V1_fit_online, V2_fit_online = online_voltage_fit(t_data, i_data, V_data, SOC_data, OCV_fn, Q, params)
...
#     dt = t_data.iloc[k] - t_data.iloc[k-1]
#     V1[k] = V1[k-1]*np.exp(-dt/R1/C1) + i_data.iloc[k-1]*R1*(1-np.exp(-dt/R1/C1))
#     V2[k] = V2[k-1]*np.exp(-dt/R2/C2) + i_data.iloc[k-1]*R2*(1-np.exp(-dt/R2/C2))
...
# V_OCV_2[0] = OCV_fn(SOC_data.iloc[0])
# for k in range(1,len(V_data)):
#     V_OCV_2[k] = OCV_fn(SOC_data.iloc[k])
...
# V0_2[0] = i_data.iloc[0] * R0
# for k in range(1,len(V_data)):
#     V0_2[k] = i_data.iloc[k] * R0
...
param_and_error_track = [R0, R1, C1, R2, C2, R2, RMS]
df_export = pd.DataFrame([param_and_error_track],columns=["Ro","R1","C1","R2","C2","R2","RMS"])
filename = f"{name}.csv"
